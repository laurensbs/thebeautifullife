...
const COPY: Record<"nl" | "en", Copy> = {
  nl: {
    loading: "een moment…",
    thanksWith: (name: string) => `dankjewel, ${name}`,
    thanks: "dankjewel",
    title: "Je mail staat klaar",
    bodyPrefix: "Open de e-mail die we je net stuurden",
    bodyTo: "naar",
    bodySuffix: "en klik op",
    cta: "Start mijn vragenlijst",
    bodyEnd:
      ". Daarmee log je meteen in op je persoonlijke pad — geen wachtwoord nodig.",
    noMail: "Geen mail gekregen?",
    helpBody:
      "Soms duurt het een minuut. Kijk ook even in je spam- of reclame-map.",
    resend: "Stuur opnieuw",
    resending: "Bezig…",
    resentDone: "Mail opnieuw verstuurd",
    loginAlt: "Je kunt opnieuw inloggen via",
    loginAltLink: "mijn-pad/login",
    backHome: "← terug naar de site",
  },
  en: {
    loading: "one moment…",
    thanksWith: (name: string) => `thank you, ${name}`,
    thanks: "thank you",
    title: "Your email is on its way",
    bodyPrefix: "Open the email we just sent",
    bodyTo: "to",
    bodySuffix: "and click",
    cta: "Start my questionnaire",
    bodyEnd:
      ". This logs you in to your personal path right away — no password needed.",
    noMail: "Didn't receive the email?",
    helpBody:
      "Sometimes it takes a minute. Also check your spam or promotions folder.",
    resend: "Resend",
    resending: "Sending…",
    resentDone: "Email resent",
    loginAlt: "You can log in again via",
    loginAltLink: "mijn-pad/login",
    backHome: "← back to the site",
  },
};

export default function CheckMailView({ locale }: { locale: Locale }) {
  const t = COPY[locale === "en" ? "en" : "nl"];
  return (
    <Suspense fallback={<PageLoader label={t.loading} />}>
      <CheckMailContent t={t} />
    </Suspense>
  );
}

function CheckMailContent({ t }: { t: Copy }) {
    ...
    <main className="max-w-[600px] mx-auto px-5 sm:px-6 py-12 sm:py-16">
      ...
      >
        ...
        <p className="font-script text-tan text-3xl sm:text-4xl">
          {firstName ? t.thanksWith(firstName) : t.thanks}
        </p>
        ...
        <p className="text-ink-soft text-[15px] leading-[1.85] max-w-md mx-auto">
          ...
          {email ? (
            <>
              {" "}
              {t.bodyTo} <span className="text-ink">{email}</span>
            </>
          ) : null}{" "}
          {t.bodySuffix} <em className="text-ink">{t.cta}</em>
          {t.bodyEnd}
        </p>
            ...
            <div className="mt-5 text-left max-w-md mx-auto bg-page/60 border border-line/60 rounded-md px-5 py-5">
              ...
              {email ? (
                ...
                >
                  ...
                  {resent ? t.resentDone : resending ? t.resending : t.resend}
                </button>
              ) : (
